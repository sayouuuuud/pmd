...
export function BoardWorkspace() {
  ...
  function addGroup(event: FormEvent<HTMLFormElement>) {
    ...
    if (!cleanTitle) {
      setGroupError('اكتب اسم منطقة التجميع أولًا')
      return
    }
    if (activeBoard.groups.some((group) => group.title === cleanTitle)) {
      setGroupError('استخدم اسمًا مختلفًا لمنطقة التجميع')
      return
    }
    setGroupError('')
  ...
  function addBoard(event: FormEvent<HTMLFormElement>) {
    ...
    if (!nextTitle) {
      setBoardError('اكتب اسم السبورة أولًا')
      return
    }
    setBoardError('')
  ...
  function addNote(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!title.trim()) {
      setNoteError('اكتب عنوان الورقة أولًا')
      return
    }
    setNoteError('')
      ...
      <div className="flex flex-col gap-3 rounded-3xl bg-surface-dark p-5 text-surface-dark-foreground">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button type="button" variant="ghost" onClick={() => setShowBoardComposer((current) => !current)} className="flex items-center justify-center gap-2 rounded-full bg-card/15 px-3 py-2 text-xs font-semibold hover:bg-card/25"><Plus className="h-4 w-4" /> سبورة جديدة</Button>
            <Button type="button" variant="default" onClick={() => setShowComposer((current) => !current)} className="flex items-center justify-center gap-2 rounded-full bg-primary px-4 py-3 text-xs font-semibold text-primary-foreground"><CirclePlus className="h-4 w-4" /> أضف Sticky Note</Button>
          </div>
        </div>
        {showBoardComposer && <form onSubmit={addBoard} noValidate className="flex flex-col gap-2 rounded-2xl bg-card/10 p-3 sm:flex-row"><label className="sr-only" htmlFor="new-board-title">اسم السبورة</label><Input id="new-board-title" autoFocus value={newBoardTitle} onChange={(event) => { setNewBoardTitle(event.target.value); if (boardError) setBoardError('') }} aria-invalid={Boolean(boardError)} aria-describedby={boardError ? 'new-board-title-error' : undefined} placeholder="مثال: إطلاق المنتج" className="min-w-0 flex-1 rounded-xl border-surface-dark-foreground/20 bg-surface-dark px-3 py-2 text-sm" />{boardError && <p id="new-board-title-error" role="alert" aria-live="assertive" aria-atomic="true" className="text-xs text-destructive sm:basis-full">{boardError}</p>}<Button type="submit" variant="default" className="rounded-xl px-4 py-2 text-xs font-semibold">إنشاء</Button></form>}
        <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-card/10 p-2"><span className="px-2 text-[11px] text-surface-dark-foreground/60">مناطق التجميع</span><Button type="button" variant="ghost" onClick={() => setGroupFilter('all')} className={`rounded-full px-3 py-1.5 text-[11px] font-semibold ${groupFilter === 'all' ? 'bg-primary text-primary-foreground' : 'bg-card/15 hover:bg-card/25'}`}>الكل <span className="mr-1 opacity-70">{activeBoard.notes.length}</span></Button>{groupOptions.map((group) => <Button type="button" variant="ghost" key={group.id} onClick={() => setGroupFilter(group.id)} className={`rounded-full px-3 py-1.5 text-[11px] font-semibold ${groupFilter === group.id ? 'bg-primary text-primary-foreground' : 'bg-card/15 hover:bg-card/25'}`}>{group.title} <span className="mr-1 opacity-70">{activeBoard.notes.filter((note) => (note.groupId ?? 'ungrouped') === group.id).length}</span></Button>)}<Button type="button" variant="ghost" onClick={() => setShowGroupComposer((current) => !current)} className="rounded-full bg-card/15 px-3 py-1.5 text-[11px] font-semibold hover:bg-card/25">+ منطقة جديدة</Button><span className="mx-1 h-5 w-px bg-surface-dark-foreground/15" /><span className="px-2 text-[11px] text-surface-dark-foreground/60">ألوان الأوراق</span>{([{ id: 'all', title: 'كل الألوان' }, { id: 'yellow', title: 'أصفر' }, { id: 'blue', title: 'أزرق' }, { id: 'green', title: 'أخضر' }, { id: 'pink', title: 'وردي' }] as { id: BoardColorFilter; title: string }[]).map((item) => <Button type="button" variant="ghost" key={item.id} onClick={() => setColorFilter(item.id)} className={`rounded-full px-3 py-1.5 text-[11px] font-semibold ${colorFilter === item.id ? 'bg-primary text-primary-foreground' : 'bg-card/15 hover:bg-card/25'}`}>{item.title} <span className="mr-1 opacity-70">{item.id === 'all' ? activeBoard.notes.length : activeBoard.notes.filter((note) => note.color === item.id).length}</span></Button>)}</div>
        {showGroupComposer && <form onSubmit={addGroup} noValidate className="flex flex-col gap-2 rounded-2xl bg-card/10 p-3 sm:flex-row"><label className="sr-only" htmlFor="new-board-group">اسم منطقة التجميع</label><Input id="new-board-group" autoFocus value={newGroupTitle} onChange={(event) => { setNewGroupTitle(event.target.value); if (groupError) setGroupError('') }} aria-invalid={Boolean(groupError)} aria-describedby={groupError ? 'new-board-group-error' : undefined} placeholder="مثال: أفكار محتوى" className="min-w-0 flex-1 rounded-xl border-surface-dark-foreground/20 bg-surface-dark px-3 py-2 text-sm" />{groupError && <p id="new-board-group-error" role="alert" aria-live="assertive" aria-atomic="true" className="text-xs text-destructive sm:basis-full">{groupError}</p>}<Button type="submit" variant="default" className="rounded-xl px-4 py-2 text-xs font-semibold">إضافة المنطقة</Button></form>}
      </div>

            {showComposer && <form onSubmit={addNote} noValidate className="rounded-3xl border border-border bg-card p-4 shadow-sm"><div className="grid gap-3 md:grid-cols-[1fr_1.4fr_150px_110px_110px_auto] md:items-end"><label className="space-y-2"><span className="text-xs font-semibold">العنوان</span><Input value={title} onChange={(event) => { setTitle(event.target.value); if (noteError) setNoteError('') }} aria-invalid={Boolean(noteError)} aria-describedby={noteError ? 'new-board-note-error' : undefined} placeholder="مثال: فكرة للويك إند" className="w-full rounded-2xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary" />{noteError && <p id="new-board-note-error" role="alert" aria-live="assertive" aria-atomic="true" className="text-xs text-destructive">{noteError}</p>}</label>
<label className="space-y-2"><span className="text-xs font-semibold">ملاحظة قصيرة</span><Input value={body} onChange={(event) => setBody(event.target.value)} placeholder="ما الخطوة أو الفكرة؟" className="w-full rounded-2xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary" /></label><label className="space-y-2"><span className="text-xs font-semibold">المكان</span><Select value={lane} onChange={(event) => setLane(event.target.value as BoardLane)} className="w-full rounded-2xl px-3 py-2.5 text-sm">{lanes.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</Select></label><label className="space-y-2"><span className="text-xs font-semibold">اللون</span><Select value={color} onChange={(event) => setColor(event.target.value as BoardColor)} className="w-full rounded-2xl border border-border bg-background px-3 py-2.5 text-sm outline-none"><option value="yellow">أصفر</option><option value="blue">أزرق</option><option value="green">أخضر</option><option value="pink">وردي</option></Select></label><label className="space-y-2"><span className="text-xs font-semibold">حجم الورقة</span><Select aria-label="حجم الورقة الجديدة" value={size} onChange={(event) => setSize(event.target.value as BoardNoteSize)} className="w-full rounded-2xl border border-border bg-background px-3 py-2.5 text-sm outline-none"><option value="small">صغير</option><option value="large">كبير</option></Select></label><label className="space-y-2"><span className="text-xs font-semibold">منطقة التجميع</span><Select value={selectedGroupId} onChange={(event) => setSelectedGroupId(event.target.value)} className="w-full rounded-2xl border border-border bg-background px-3 py-2.5 text-sm outline-none"><option value="">بدون مجموعة</option>{activeBoard.groups.map((group) => <option key={group.id} value={group.id}>{group.title}</option>)}</Select></label><Button type="submit" variant="default" className="flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-xs font-semibold"><Plus className="h-4 w-4" /> إضافة</Button></div></form>}
...
const noteColors: Record<BoardColor, string> = { yellow: 'bg-board-note-yellow', blue: 'bg-board-note-blue', green: 'bg-board-note-green', pink: 'bg-board-note-pink' }
...
function BoardNoteCard({ note, groupTitle, groupOptions, onMove, onAssignGroup, onResize, onArchive, onConvertToTask, onConvertToNote, onDragEnd }: { note: BoardNote; groupTitle: string; groupOptions: BoardGroup[]; onMove: (id: string, lane: BoardLane) => void; onAssignGroup: (id: string, groupId: string) => void; onResize: (id: string, size: BoardNoteSize) => void; onArchive: (id: string) => void; onConvertToTask: (id: string) => void; onConvertToNote: (id: string) => void; onDragEnd: (event: React.DragEvent<HTMLElement>) => void }) {
  ...
  return <article draggable onDragEnd={onDragEnd} style={{ left: note.x, top: note.y }} className={`group absolute ${noteSizes[note.size]} rounded-2xl p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${noteColors[note.color]}`}><div className="flex items-start justify-between gap-2"><div><h3 className="text-sm font-bold text-board-note-ink">{note.title}</h3>{note.body && <p className="mt-2 text-xs leading-6 text-board-note-muted/80">{note.body}</p>}</div><Button type="button" variant="ghost" aria-label="أرشفة الملاحظة" onClick={() => onArchive(note.id)} className="rounded-full p-1 text-board-note-muted opacity-0 transition hover:bg-board-note-ink/5 group-hover:opacity-100 focus-visible:opacity-100"><Archive className="h-3.5 w-3.5" /></Button></div><div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-[10px] text-board-note-muted/70"><span>{note.createdAt}</span>{note.lane !== 'done' && <Button type="button" variant="ghost" onClick={() => onMove(note.id, nextLane)} className="flex items-center gap-1 rounded-full bg-board-note-soft/50 px-2 py-1 font-semibold hover:bg-board-note-soft/80">للمرحلة التالية <ArrowLeft className="h-3 w-3" /></Button>}</div><div className="mt-3 flex flex-wrap items-center gap-1"><span className="inline-flex rounded-full bg-board-note-soft/40 px-2 py-1 text-[10px] text-board-note-muted">{lanes[currentIndex].title}</span><span className="inline-flex rounded-full bg-board-note-soft/40 px-2 py-1 text-[10px] text-board-note-muted">{groupTitle}</span><Select aria-label={`منطقة تجميع ${note.title}`} value={note.groupId ?? 'ungrouped'} onChange={(event) => onAssignGroup(note.id, event.target.value)} onPointerDown={(event) => event.stopPropagation()} className="max-w-28 rounded-full bg-board-note-soft/50 px-2 py-1 text-[10px] text-board-note-ink outline-none"><option value="ungrouped">بدون مجموعة</option>{groupOptions.filter((group) => group.id !== 'ungrouped').map((group) => <option key={group.id} value={group.id}>{group.title}</option>)}</Select><Select aria-label={`حجم ورقة ${note.title}`} value={note.size} onChange={(event) => onResize(note.id, event.target.value as BoardNoteSize)} onPointerDown={(event) => event.stopPropagation()} className="rounded-full bg-board-note-soft/50 px-2 py-1 text-[10px] text-board-note-ink outline-none"><option value="small">صغير</option><option value="large">كبير</option></Select></div><div className="mt-3 flex flex-wrap gap-1 border-t border-board-note-border/10 pt-2"><Button type="button" variant="ghost" onClick={() => onConvertToTask(note.id)} className="rounded-full bg-board-note-soft/50 px-2 py-1 text-[10px] font-semibold text-board-note-ink hover:bg-board-note-soft/80">تحويل لمهمة</Button><Button type="button" variant="ghost" onClick={() => onConvertToNote(note.id)} className="rounded-full bg-board-note-soft/50 px-2 py-1 text-[10px] font-semibold text-board-note-ink hover:bg-board-note-soft/80">حفظ كملاحظة</Button></div></article>
}
