  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-3xl bg-surface-dark p-5 text-surface-dark-foreground">
        ...
        {showBoardComposer && <form onSubmit={addBoard} noValidate className="flex flex-col gap-2 rounded-2xl bg-card/10 p-3 sm:flex-row"><label className="sr-only" htmlFor="new-board-title">اسم السبورة</label><Input id="new-board-title" autoFocus value={newBoardTitle} onChange={(event) => { setNewBoardTitle(event.target.value); if (boardError) setBoardError('') }} aria-invalid={Boolean(boardError)} aria-describedby={boardError ? 'new-board-title-error' : undefined} placeholder="مثال: إطلاق المنتج" className="min-w-0 flex-1 rounded-xl border-surface-dark-foreground/20 bg-surface-dark px-3 py-2 text-sm" />{boardError && <p id="new-board-title-error" role="alert" className="text-xs text-destructive sm:basis-full">{boardError}</p>}<Button type="submit" variant="default" className="rounded-xl px-4 py-2 text-xs font-semibold">إنشاء</Button></form>}
