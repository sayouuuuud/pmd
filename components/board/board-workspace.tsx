...
type BoardNote = { id: string; title: string; body: string; lane: BoardLane; color: BoardColor; groupId?: string; createdAt: string; x: number; y: number }
...
const seedNotes: BoardNote[] = [
  { id: 'board-1', title: 'فكرة للمنتج', body: 'أضيف مساحة تجمع قرارات المراجعة الأسبوعية.', lane: 'ideas', color: 'yellow', groupId: 'group-capture', createdAt: 'اليوم', x: 36, y: 36 },
  { id: 'board-2', title: 'جلسة تركيز', body: 'أبدأ بخطوة عميقة واحدة قبل فتح الرسائل.', lane: 'next', color: 'blue', groupId: 'group-week', createdAt: 'اليوم', x: 300, y: 36 },
  { id: 'board-3', title: 'منصة التحكم الشخصي', body: 'ربط المجالات الأساسية في لوحة واحدة.', lane: 'doing', color: 'green', groupId: 'group-decisions', createdAt: 'هذا الأسبوع', x: 564, y: 36 },
]
...
export function BoardWorkspace() {
  ...
  const visibleNotes = groupFilter === 'all' ? activeBoard.notes : activeBoard.notes.filter((note) => (note.groupId ?? 'ungrouped') === groupFilter)
  ...
  function addNote(event: FormEvent<HTMLFormElement>) {
    ...
    updateActiveNotes((current) => [{ id: `board-${Date.now()}`, title: title.trim(), body: body.trim(), lane, color, groupId: selectedGroupId || undefined, createdAt: 'الآن', x: 36 + (index % 3) * 264, y: 36 + Math.floor(index / 3) * 170 }, ...current])
    setTitle('')
    setBody('')
...
function normalizeNotes(value: unknown[]): BoardNote[] {
  return value.flatMap((item, index) => {
    ...
    return [{ id: item.id, title: item.title, body: typeof item.body === 'string' ? item.body : '', lane, color, groupId: typeof item.groupId === 'string' ? item.groupId : undefined, createdAt: typeof item.createdAt === 'string' ? item.createdAt : 'قديم', x: typeof item.x === 'number' ? item.x : 36 + (index % 3) * 264, y: typeof item.y === 'number' ? item.y : 36 + Math.floor(index / 3) * 170 }]
  })
}
