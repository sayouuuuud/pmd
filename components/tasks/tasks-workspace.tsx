...
import { AlertTriangle, Archive, CalendarDays, Check, ChevronDown, Clock3, Plus, Repeat2, Search, Trash2 } from 'lucide-react'
...
export function TasksWorkspace() {
  const { tasks, toggleTask, addTask, updateTask, archiveTask, addSubtask, toggleSubtask, removeSubtask } = useCommandCenter()
  ...
  return <div className="grid min-w-0 grid-cols-1 gap-4 xl:grid-cols-12">
    <ContentCard className="xl:col-span-8" title="المهام" description={`${tasks.filter((task) => task.status !== 'done').length} مفتوحة · ${tasks.filter((task) => isTaskOverdue(task)).length} متأخرة`}>
      ...
      {visibleTasks.length === 0 ? <div className="mt-5"><EmptyState icon={CalendarDays} title="لا توجد مهام هنا" description="أضف مهمة بموعد فعلي أو غيّر البحث والفلاتر." /></div> : <div className="mt-5 flex flex-col gap-5">
        {overdue.length > 0 && <TaskSection title="متأخرة" icon={<AlertTriangle className="h-4 w-4 text-destructive" />} tasks={overdue} {...{toggleTask,updateTask,archiveTask,addSubtask,toggleSubtask,removeSubtask}} />}
        {upcoming.length > 0 && <TaskSection title="القادمة" icon={<CalendarDays className="h-4 w-4 text-primary" />} tasks={upcoming} {...{toggleTask,updateTask,archiveTask,addSubtask,toggleSubtask,removeSubtask}} />}
      </div>}
    </ContentCard>
...
function TaskSection({ title, icon, tasks, ...actions }: { title: string; icon: React.ReactNode; tasks: Task[] } & Actions) {
  return <section><div className="mb-2 flex items-center gap-2"><span>{icon}</span><h2 className="text-sm font-bold">{title}</h2><span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">{tasks.length}</span></div><div className="flex flex-col gap-2">{tasks.map((task) => <TaskRow key={task.id} task={task} {...actions} />)}</div></section>
}

function TaskRow({ task, toggleTask, updateTask, archiveTask, addSubtask, toggleSubtask, removeSubtask }: { task: Task } & Actions) {
