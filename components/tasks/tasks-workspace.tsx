...
export function TasksWorkspace() {
  ...
  const visibleTasks = useMemo(() => tasks.filter((task) => {
    ...
    return matchesQuery && matchesFilter && matchesPriority && matchesCategory
  }), [tasks, filter, query, priorityFilter, categoryFilter])
  ...
  function createTask(event: React.FormEvent<HTMLFormElement>) {
    ...
    addTask({ title: newTitle.trim(), priority, dueLabel: newRecurrence === 'weekly' ? 'هذا الأسبوع' : 'النهاردة', category: 'عام', recurring: newRecurrence !== 'none' })
    setNewTitle('')
...
function TaskRow({ task, projectId, projectTitle, goalId, goalTitle, onToggle, onArchive, onUpdate, onMove, onAddSubtask, onToggleSubtask, onRemoveSubtask }: TaskRowProps) {
  ...
  function saveEditing(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!editTitle.trim() || !editCategory.trim() || !editDueLabel.trim()) return
