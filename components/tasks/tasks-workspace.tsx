...
export function TasksWorkspace() {
  ...
  const [sortBy, setSortBy] = useState<'priority' | 'title' | 'status'>('priority')
  ...
  const visibleTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      const matchesQuery = !normalizedQuery || `${task.title} ${task.description ?? ''} ${task.category} ${(task.subtasks ?? []).map((subtask) => subtask.title).join(' ')}`.includes(normalizedQuery)
      ...
      if (sortBy === 'status') return statusOrder[left.status] - statusOrder[right.status]
      return priorityOrder[left.priority] - priorityOrder[right.priority]
    })
  }, [tasks, filter, query, statusFilter, priorityFilter, categoryFilter, sortBy])
  ...
  return <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
    <ContentCard className="xl:col-span-8" title="قائمة المهام" description={`${tasks.filter((task) => task.status !== 'done').length} مهام مفتوحة`} action={<div className="flex items-center gap-2 text-xs text-muted-foreground"><SlidersHorizontal className="h-4 w-4" /> مرتبة ��سب الأولوية</div>}>
