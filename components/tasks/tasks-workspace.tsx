...
import { Archive, Check, ChevronDown, Filter, ListPlus, Plus, Search, SlidersHorizontal, Trash2 } from 'lucide-react'
...
export function TasksWorkspace() {
  const { tasks, toggleTask, addTask, archiveTask, addSubtask, toggleSubtask, removeSubtask } = useCommandCenter()
  ...
  return <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
    <ContentCard className="lg:col-span-8" title="قائمة المهام" description={`${tasks.filter((task) => task.status !== 'done').length} مهام مفتوحة`} action={<div className="flex items-center gap-2 text-xs text-muted-foreground"><SlidersHorizontal className="h-4 w-4" /> مرتبة حسب الأولوية</div>}>
      ...
      <div className="mt-5 space-y-2">{visibleTasks.map((task) => <TaskRow key={task.id} task={task} onToggle={() => toggleTask(task.id)} onArchive={() => archiveTask(task.id)} onAddSubtask={(title) => addSubtask(task.id, title)} onToggleSubtask={(subtaskId) => toggleSubtask(task.id, subtaskId)} onRemoveSubtask={(subtaskId) => removeSubtask(task.id, subtaskId)} />)}{visibleTasks.length === 0 && <div className="rounded-2xl bg-muted px-4 py-12 text-center text-sm text-muted-foreground">مفيش مهام مطابقة للبحث الحالي.</div>}</div>
    </ContentCard>
...
function TaskRow({ task, onToggle, onArchive, onAddSubtask, onToggleSubtask, onRemoveSubtask }: TaskRowProps) {
