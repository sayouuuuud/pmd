...
export function GlobalSearchDialog({ open, onClose }: GlobalSearchDialogProps) {
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    return [
      ...
      ...projects.filter((item) => matches(item.title) || matches(item.description) || matches(item.dueLabel)).map((item) => ({ id: item.id, title: item.title, subtitle: `مشروع · ${item.dueLabel}`, section: 'المشاريع', href: `/projects?project=${encodeURIComponent(item.id)}` })),
      ...financeEntries.filter((item) => matches(item.title) || matches(item.category) || matches(item.note ?? '')).map((item) => ({ id: item.id, title: item.title, subtitle: `${item.category} · ${item.amount.toLocaleString('ar-EG')} جنيه`, section: 'الفلوس', href: `/money?month=${encodeURIComponent(item.localDate.slice(0, 7))}` })),
      ...
      ...journal.filter((item) => matches(item.title) || matches(item.body) || matches(item.mood) || matches(item.localDate)).map((item) => ({ id: item.id, title: item.title || 'يوميات بلا عنوان', subtitle: `اليوميات · ${item.localDate} · ${item.mood}`, section: 'اليوميات', href: `/journal?date=${encodeURIComponent(item.localDate)}` })),
