...
import { CalendarDays, ChevronLeft, ChevronRight, Clock3, ExternalLink, Plus, Trash2, X } from 'lucide-react'
...
export function CalendarWorkspace() {
  ...
  async function saveEvent(event: React.FormEvent<HTMLFormElement>) {
    ...
    const localEvent: CalendarEvent = { id: makeId(), title, description: form.description.trim() || undefined, kind: form.kind as CalendarEvent['kind'], startsAt, endsAt, timezone: 'Africa/Cairo', sourceType: 'manual', status: 'planned' }
    setSaving(true)
    ...
    try {
      ...
      if (response.ok) {
        const payload = await response.json() as { item?: CalendarEvent }
        if (payload.item) setCustomEvents((current) => current.map((item) => item.id === localEvent.id ? payload.item! : item))
      }
    } catch {
  ...
  return <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
    ...
    {showForm && <ContentCard className="lg:col-span-12" title="إضافة حدث" description="سيُحفظ على الخادم عند توفره، ويظل محليًا عند غياب الاتصال."><form onSubmit={saveEvent} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><label className="lg:col-span-2 text-sm font-medium">العنوان<Input autoFocus value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} className="mt-2 rounded-2xl" placeholder="مثال: جلسة تركيز" /></label><label className="text-sm font-medium">النوع<Select value={form.kind} onChange={(event) => setForm((current) => ({ ...current, kind: event.target.value }))} className="mt-2 rounded-2xl">{eventKinds.map((kind) => <option key={kind.value} value={kind.value}>{kind.label}</option>)}</Select></label><label className="text-sm font-medium">التاريخ<Input type="date" value={form.date} onChange={(event) => setForm((current) => ({ ...current, date: event.target.value }))} className="mt-2 rounded-2xl" /></label><label className="text-sm font-medium">من<Input type="time" value={form.start} onChange={(event) => setForm((current) => ({ ...current, start: event.target.value }))} className="mt-2 rounded-2xl" /></label><label className="text-sm font-medium">إلى <span className="font-normal text-muted-foreground">(اختياري)</span><Input type="time" value={form.end} onChange={(event) => setForm((current) => ({ ...current, end: event.target.value }))} className="mt-2 rounded-2xl" /></label><label className="sm:col-span-2 lg:col-span-4 text-sm font-medium">وصف مختصر<Textarea value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} className="mt-2 min-h-20 rounded-2xl" placeholder="تفاصيل تساعدك عندما ترى الحدث لاحقًا" /></label><div className="flex items-end gap-2 lg:col-span-2"><Button type="submit" disabled={saving}>{saving ? 'جارٍ الحفظ...' : 'حفظ الحدث'}</Button><Button type="button" variant="outline" onClick={() => setShowForm(false)}><X className="ml-1 h-4 w-4" />إلغاء</Button></div>{error && <p className="sm:col-span-2 lg:col-span-6 rounded-xl bg-destructive/10 px-3 py-2 text-xs text-destructive" role="alert">{error}</p>}</form></ContentCard>}
  </div>
}
