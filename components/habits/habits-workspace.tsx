...
export function HabitsWorkspace() {
  ...
  return <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
    <ContentCard className="xl:col-span-8" title="عادات النهاردة" description={`${completed} من ${habits.length} عادات مكتملة`} action={<Button type="button" onClick={() => setShowForm((visible) => !visible)} aria-expanded={showForm} size="sm" className="gap-2 rounded-full text-xs font-semibold"><Plus className="h-3.5 w-3.5" /> عادة جديدة</Button>}>
      ...
      {habits.length === 0 ? <EmptyState icon={Repeat} title="لا توجد عادات بعد" description="ابدأ بعادة صغيرة مرتبطة بهدف أو مشروع حتى يظهر أثرها في خطة اليوم." action={<Button type="button" onClick={() => setShowForm(true)} size="sm" className="rounded-xl text-xs font-semibold">أضف أول عادة</Button>} /> : <div className="grid gap-3 sm:grid-cols-2">{habits.map((habit) => {
        ...
        return <div key={habit.id} id={habit.id} className="scroll-mt-24 rounded-2xl border border-border/70 bg-muted/60 p-4 transition-all hover:-translate-y-0.5 hover:bg-card">
          ...
          {(task || project || goal) && <div className="mt-3 flex flex-wrap gap-1.5 text-[10px]"><span className="inline-flex items-center gap-1 text-muted-foreground"><Link2 className="h-3 w-3 text-primary" />مرتبط بـ</span>{task && <Link href={contextHref('task', task.id)} className="rounded-full bg-accent px-2 py-1 text-accent-foreground hover:underline">مهمة: {task.title}</Link>}{project && <Link href={contextHref('project', project.id)} className="rounded-full bg-card px-2 py-1 hover:underline">مشروع: {project.title}</Link>}{goal && <Link href={contextHref('goal', goal.id)} className="rounded-full bg-primary/10 px-2 py-1 text-primary hover:underline">هدف: {goal.title}</Link>}</div>}
