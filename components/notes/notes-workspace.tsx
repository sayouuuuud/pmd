...
export function NotesWorkspace() {
  ...
  useEffect(() => {
    const saved = window.sessionStorage.getItem('notes-view')
  ...
  function createNote(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNoteError('')
    addNote({ title: title.trim(), body: body.trim() || 'ملاحظة سريعة', tag: joinNoteTags(selectedTags), projectId: projectId || undefined })
    setTitle('')
    setBody('')
    setProjectId('')
...
function NoteCard({ note, taskId, taskTitle, projectId, projectTitle, goalId, goalTitle, onUpdate, onRestore, onPin, onArchive }: NoteCardProps) {
  ...
  return <article id={`note-${note.id}`} className="group scroll-mt-24 rounded-2xl border border-border/70 bg-muted/60 p-4 transition-colors hover:bg-card">
    ...
    {editing && <form onSubmit={saveEditing} noValidate className="mt-3 grid gap-2 border-t border-border/70 pt-3"><Input value={editTitle} onChange={(event) => { setEditTitle(event.target.value); if (editError) setEditError('') }} aria-label={`عنوان الملاحظة ${note.title}`} aria-invalid={Boolean(editError)} aria-describedby={editError ? `note-${note.id}-edit-error` : undefined} className="h-auto rounded-xl px-3 py-2 text-xs" placeholder="عنوان الملاحظة" />{editError && <p id={`note-${note.id}-edit-error`} role="alert" aria-live="assertive" aria-atomic="true" className="text-xs text-destructive">{editError}</p>}<Textarea value={editBody} onChange={(event) => setEditBody(event.target.value)} aria-label={`محتوى الملاحظة ${note.title}`} className="min-h-28 rounded-xl px-3 py-2 text-xs leading-6" placeholder="محتوى الملاحظة" /><div className="grid grid-cols-3 gap-1 rounded-xl border border-input bg-background p-1">{noteTags.map((tag) => <Button key={tag} type="button" variant="ghost" size="xs" onClick={() => setEditTags((items) => toggleTag(items, tag))} aria-pressed={editTags.includes(tag)} className={`h-auto rounded-lg px-2 py-1.5 text-[10px] ${editTags.includes(tag) ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-accent'}`}>{tag}</Button>)}</div><div className="flex justify-end gap-2"><Button type="submit" aria-label="حفظ تعديل الملاحظة" className="h-auto rounded-xl px-3 py-2 text-xs font-semibold"><Save className="h-3.5 w-3.5" /> حفظ</Button><Button type="button" variant="outline" onClick={() => setEditing(false)} aria-label="إلغاء تعديل الملاحظة" className="h-auto rounded-xl px-3 py-2 text-xs text-muted-foreground"><X className="h-3.5 w-3.5" /> إلغاء</Button></div></form>}
    {note.versions && note.versions.length > 0 && <div className="mt-3 border-t border-border/70 pt-3"><Button type="button" variant="ghost" size="xs" onClick={() => setShowVersions((value) => !value)} className="rounded-lg text-[10px] text-muted-foreground">{showVersions ? 'إخفاء سجل النسخ' : `سجل النسخ (${note.versions.length})`}</Button>{showVersions && <div className="mt-2 grid gap-1.5">{note.versions.map((version) => <div key={version.id} className="flex items-center justify-between gap-2 rounded-xl bg-card px-3 py-2 text-[10px]"><span className="truncate text-muted-foreground">{version.title} · {new Date(version.savedAt).toLocaleString('ar-EG')}</span><Button type="button" variant="outline" size="xs" onClick={() => onRestore(version.id)} className="shrink-0 rounded-lg">استعادة</Button></div>)}</div>}</div>}
    {(taskTitle || projectTitle || goalTitle) && <div className="mt-3 flex flex-wrap gap-1.5 text-[10px]"><span className="inline-flex items-center gap-1 text-muted-foreground"><Link2 className="h-3 w-3 text-primary" />مرتبط بـ</span>{taskTitle && taskId && <Link href={contextHref('task', taskId)} className="rounded-full bg-accent px-2 py-1 text-accent-foreground hover:underline">مهمة: {taskTitle}</Link>}{projectTitle && projectId && <Link href={contextHref('project', projectId)} className="rounded-full bg-card px-2 py-1 hover:underline">مشروع: {projectTitle}</Link>}{goalTitle && goalId && <Link href={contextHref('goal', goalId)} className="rounded-full bg-primary/10 px-2 py-1 text-primary hover:underline">هدف: {goalTitle}</Link>}</div>}
