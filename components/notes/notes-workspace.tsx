  ...
  return <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
    <ContentCard className="lg:col-span-8" title="ملاحظاتك" description={`${notes.length} ملاحظات محفوظة`} action={<div className="flex items-center gap-1 rounded-xl bg-muted p-1"><button type="button" onClick={() => setView('grid')} aria-label="شبكة" className={`rounded-lg p-2 ${view === 'grid' ? 'bg-card shadow-sm' : 'text-muted-foreground'}`}><Grid2X2 className="h-4 w-4" /></button><button type="button" onClick={() => setView('list')} aria-label="قائمة" className={`rounded-lg p-2 ${view === 'list' ? 'bg-card shadow-sm' : 'text-muted-foreground'}`}><List className="h-4 w-4" /></button></div>}>
      ...
      <div className={view === 'grid' ? 'mt-5 grid gap-3 sm:grid-cols-2' : 'mt-5 space-y-2'}>{visibleNotes.map((note) => { const task = note.sourceTaskId ? tasks.find((item) => item.id === note.sourceTaskId) : undefined; const project = task?.projectId ? projects.find((item) => item.id === task.projectId) : undefined; const goal = project?.goalId ? goals.find((item) => item.id === project.goalId) : undefined; return <NoteCard key={note.id} note={note} taskId={task?.id} taskTitle={task?.title} projectId={project?.id} projectTitle={project?.title} goalId={goal?.id} goalTitle={goal?.title} onUpdate={(patch) => updateNote(note.id, patch)} onPin={() => toggleNotePin(note.id)} onArchive={() => archiveNote(note.id)} /> })}{visibleNotes.length === 0 && <div className="col-span-full"><EmptyState icon={FileText} title="لا توجد ملاحظات مطابقة" description="جرّب تغيير البحث أو وسم الفلترة، أو اكتب ملاحظة سريعة لتبدأ مساحة الالتقاط." /></div>}</div>
          ...
          <ContentCard className="lg:col-span-4" title="التقاط سريع" description="اكتب الفكرة كما هي، ورتّبها لاحقًا."><form onSubmit={createNote}><Input value={title} onChange={(event) => setTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit() } }} aria-keyshortcuts="Enter" aria-label="عنوان الملاحظة" className="rounded-2xl px-4 py-3" placeholder="عنوان الملاحظة — اضغط Enter للحفظ السريع" /><Textarea value={body} onChange={(event) => setBody(event.target.value)} aria-label="تفاصيل الملاحظة" className="mt-3 min-h-32 rounded-2xl px-4 py-3 leading-6" placeholder="اكتب التفاصيل..." /><div className="mt-3 grid grid-cols-3 gap-2">{noteTags.map((tag) => <button type="button" key={tag} onClick={() => setSelectedTags((items) => toggleTag(items, tag))} aria-pressed={selectedTags.includes(tag)} className={`rounded-xl px-2 py-2 text-xs ${selectedTags.includes(tag) ? 'bg-accent font-semibold text-accent-foreground' : 'bg-muted text-muted-foreground'}`}>{tag}</button>)}</div><p className="mt-2 text-[11px] text-muted-foreground">يمكن اختيار أكثر من وسم للملاحظة. اضغط Enter داخل العنوان للحفظ السريع.</p><Button type="submit" className="mt-3 h-auto w-full rounded-2xl px-4 py-3 text-sm font-semibold"><Plus className="h-4 w-4" /> حفظ الملاحظة</Button></form></ContentCard>
  </div>
}
...
function NoteCard({ note, taskId, taskTitle, projectId, projectTitle, goalId, goalTitle, onUpdate, onPin, onArchive }: NoteCardProps) {
  ...
  return <article id={`note-${note.id}`} className="group scroll-mt-24 rounded-2xl border border-border/70 bg-muted/60 p-4 transition-colors hover:bg-card">
    ...
    {editing && <form onSubmit={saveEditing} className="mt-3 grid gap-2 border-t border-border/70 pt-3"><Input value={editTitle} onChange={(event) => setEditTitle(event.target.value)} aria-label={`عنوان الملاحظة ${note.title}`} className="h-auto rounded-xl px-3 py-2 text-xs" placeholder="عنوان الملاحظة" /><Textarea value={editBody} onChange={(event) => setEditBody(event.target.value)} aria-label={`محتوى الملاحظة ${note.title}`} className="min-h-28 rounded-xl px-3 py-2 text-xs leading-6" placeholder="محتوى الملاحظة" /><div className="grid grid-cols-3 gap-1 rounded-xl border border-input bg-background p-1">{noteTags.map((tag) => <button key={tag} type="button" onClick={() => setEditTags((items) => toggleTag(items, tag))} aria-pressed={editTags.includes(tag)} className={`rounded-lg px-2 py-1.5 text-[10px] ${editTags.includes(tag) ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-accent'}`}>{tag}</button>)}</div><div className="flex justify-end gap-2"><Button type="submit" aria-label="حفظ تعديل الملاحظة" className="h-auto rounded-xl px-3 py-2 text-xs font-semibold"><Save className="h-3.5 w-3.5" /> حفظ</Button><Button type="button" variant="outline" onClick={() => setEditing(false)} aria-label="إلغاء تعديل الملاحظة" className="h-auto rounded-xl px-3 py-2 text-xs text-muted-foreground"><X className="h-3.5 w-3.5" /> إلغاء</Button></div></form>}
