...
function NoteCard({ note, taskId, taskTitle, projectId, projectTitle, goalId, goalTitle, onPin, onArchive }: { note: Note; taskId?: string; taskTitle?: string; projectId?: string; projectTitle?: string; goalId?: string; goalTitle?: string; onPin: () => void; onArchive: () => void }) {
  return <article id={`note-${note.id}`} className="group scroll-mt-24 rounded-2xl border border-border/70 bg-muted/60 p-4 transition-colors hover:bg-card"><div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-accent text-accent-foreground"><FileText className="h-4 w-4" /></span><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-semibold">{note.title}</h3><p className="mt-1 line-clamp-3 text-xs leading-6 text-muted-foreground">{note.body}</p></div><button type="button" onClick={onPin} aria-label={note.pinned ? 'إلغاء التثبيت' : 'تثبيت'} className="rounded-full p-1.5 text-muted-foreground hover:bg-muted">{note.pinned ? <BookmarkCheck className="h-4 w-4 text-primary" /> : <Bookmark className="h-4 w-4" />}</button></div>{(taskTitle || projectTitle || goalTitle) && <div className="mt-3 flex flex-wrap gap-1.5 text-[10px]"><span className="inline-flex items-center gap-1 text-muted-foreground"><Link2 className="h-3 w-3 text-primary" />مرتبط بـ</span>{taskTitle && taskId && <Link href={`/tasks#task-${taskId}`} className="rounded-full bg-accent px-2 py-1 text-accent-foreground hover:underline">مهمة: {taskTitle}</Link>}{projectTitle && projectId && <Link href={`/projects#${projectId}`} className="rounded-full bg-card px-2 py-1 hover:underline">مشروع: {projectTitle}</Link>}{goalTitle && goalId && <Link href={`/goals#${goalId}`} className="rounded-full bg-primary/10 px-2 py-1 text-primary hover:underline">هدف: {goalTitle}</Link>}</div>}<div className="mt-4 flex items-center justify-between text-[11px] text-muted-foreground"><span className="rounded-full bg-card px-2 py-1">{note.tag}</span><span>{note.createdAt}</span><button type="button" onClick={onArchive} aria-label="أرشفة الملاحظة" className="opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100"><Archive className="h-3.5 w-3.5" /></button></div></article>
}
