...
import { BookOpen, Check, Clock3, Compass, MapPin, Moon, Sunrise, SunMedium } from 'lucide-react'
...
export function ReligiousWorkspace() {
  const { religious, togglePrayer, addWirdProgress, toggleDhikr, updateReligiousSettings } = useCommandCenter()
    ...
    <div className="space-y-5">
      <ContentCard className="bg-surface-dark text-surface-dark-foreground" title="رفقًا بنفسك، خطوة ثابتة تكفي" description="مساحة هادئة لمتابعة عبادتك اليومية دون ضغط أو أحكام تلقائية.">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="rounded-full bg-primary/15 px-3 py-1.5 text-xs font-medium text-primary">بيانات المواقيت قابلة للتحديث</span>
        </div>
        <p className="mt-3 text-xs text-surface-dark-foreground/60">يتم حفظ تقدمك وإعداداتك فقط. نصوص القرآن والأذكار ومصادر المواقيت الخارجية لا تُخزّن محليًا كنص موثوق.</p>
      </ContentCard>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <ContentCard className="lg:col-span-2" title="إعدادات المواقيت" description="الإعدادات الحالية محلية وتُزامن عند توفر الحساب وقاعدة البيانات.">
          <div className="grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium"><span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" /> المدينة</span><input value={religious.city} onChange={(event) => updateReligiousSettings({ city: event.target.value, calculationMethod: religious.calculationMethod })} className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-primary" /></label><label className="space-y-2 text-sm font-medium"><span>طريقة الحساب</span><select value={religious.calculationMethod} onChange={(event) => updateReligiousSettings({ city: religious.city, calculationMethod: event.target.value })} className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-primary"><option>مخصص</option><option>الهيئة المصرية العامة للمساحة</option><option>رابطة العالم الإسلامي</option></select></label></div>
          <div className="mt-4 flex items-start gap-2 rounded-2xl bg-muted/60 p-3 text-xs text-muted-foreground"><Sunrise className="mt-0.5 h-4 w-4 shrink-0 text-primary" /> عند تفعيل مصدر مواقيت موثوق لاحقًا، ستُستبدل القيم التجريبية تلقائيًا دون فقدان سجل الإنجاز.</div>
        </ContentCard>
      </div>
    </div>
  )
