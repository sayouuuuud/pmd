...
## Projects / Pricing / Finance — تحصيل الدفعات وربط الدخل — 2026-08-17

| المجال | الحالة | الدليل | حدود الاعتماد |
|---|---|---|---|
| واجهة التسعير | PASS تجريبي | صفحة `/projects` أظهرت تبويب «التسعير والدفعات» وزر «تم التحصيل وربطه» داخل الهوية العربية RTL الحالية. | لم تُختبر جلسة مستخدم حقيقية في هذه البيئة. |
| fallback المحلي | PASS | دفعة اختبار بقيمة 321 جنيهًا تحولت إلى «تم التحصيل»، وحُفظ دخل محلي مع رسالة مزامنة قابلة لإعادة المحاولة دون سجل محلي مكرر. | يظل السجل المحلي مؤقتًا حتى نجاح backend. |
| endpoint التحصيل | PASS بنيوي | أُضيف `POST /api/projects/pricing/[pricingId]/collect` مع session check وملكية `user_id` للمشروع والدفعة، ورفض الدفعات الملغاة. | اختبار HTTP الموثق بقاعدة بيانات حقيقية مؤجل لغياب `DATABASE_URL`. |
| الذرية وidempotency | PASS بنيوي | إنشاء/استعادة سجل الدخل وتحديث الدفعة داخل `db.transaction`، مع معرف ثابت `pricing-income-{pricingId}` وإعادة استخدام الرابط الموجود. | يلزم اختبار race condition وقيود قاعدة البيانات الفعلية قبل الإنتاج. |
